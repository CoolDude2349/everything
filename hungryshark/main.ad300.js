window.boot = function () {
    ...
    function setLoadingDisplay () {
        ...
        cc.director.once(cc.Director.EVENT_AFTER_SCENE_LAUNCH, function () {
            ...
            document.getElementById('ubi_cocos-logo').style.display = 'none';
    ...
    var onStart = function () {
        ...
        if (cc.sys.isMobile) {
            ...
            // Cita rem to fix bug HS-1449 [NCF][NAV][Compat][Android] Edit name will make game enter full screen mode
        ...
        window.firebaseInfo.firebaseAnalytics.logEvent( "first_scene_load_start",{"clientTimestamp":Date.now()});

        bundle.loadScene(launchScene, null, onProgress,
            function (err, scene) {
                if (!err) {
                    window.firebaseInfo.firebaseAnalytics.logEvent( "first_scene_load_end",{"clientTimestamp":Date.now()});
    ...
    function cb (err) {
        ...
        if (count === bundleRoot.length + 1) {
            window.firebaseInfo.firebaseAnalytics.logEvent( "project_code_load_start",{"clientTimestamp":Date.now()});
